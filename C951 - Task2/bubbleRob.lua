function sysCall_init()
    ...
    noseSensor0=sim.getObjectHandle("bubbleRob_sensingNose0") -- Handle of the proximity sensor
...
function sysCall_actuation() 
    ...
    result0=sim.readProximitySensor(noseSensor0) -- Read the proximity sensor
    -- If we detected something, we set the backward mode:
    if (result>0) then backUntilTime=sim.getSimulationTime()+4 end 
    
    if (result0>0) then
        print("THE VICTIM HAS BEEN FOUND")
    end
